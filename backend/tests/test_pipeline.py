...
def test_process_meeting_pipeline_success():
    ...
    mock_update.data = [
        {
            ...
            "action_items": [
                {"task": "Check tests", "owner": "Bob", "deadline": "Friday"}
            ],
            "processing_time": 4.5,
    ...
    with patch(
        "app.services.pipeline.download_audio_file", return_value=b"fake_audio_bytes"
    ) as mock_dl, patch(
        "app.services.pipeline.transcribe_audio",
        return_value=("Alice says let us deploy on Monday.", 2.1),
    ) as mock_asr, patch(
        "app.services.pipeline.generate_meeting_summary"
    ) as mock_sum:
        ...
        mock_sum.return_value = (
            MeetingSummaryOutput(
                ...
                decisions=["Deploy on Monday"],
                action_items=[
                    ActionItem(task="Check tests", owner="Bob", deadline="Friday")
                ],
            ),
            1.9,
        )
        ...
        mock_dl.assert_called_once_with(
            "meetings/test-pipe-1/audio.wav", client=mock_db
        )
...
def test_process_meeting_pipeline_already_completed():
    mock_db = MagicMock()
    mock_select = MagicMock()
    mock_select.data = [{"meeting_id": "already-done", "status": "COMPLETED"}]
    mock_db.table.return_value.select.return_value.eq.return_value.execute.return_value = (
        mock_select
    )

    result = process_meeting_pipeline(
        meeting_id="already-done", supabase_client=mock_db
    )
    assert result["status"] == "COMPLETED"
...
def test_process_meeting_pipeline_storage_download_failure():
    ...
    with patch(
        "app.services.pipeline.download_audio_file",
        side_effect=StorageError("Storage connection reset"),
    ):
        with pytest.raises(PipelineError) as exc:
...
def test_process_meeting_pipeline_transcription_failure():
    ...
    with patch(
        "app.services.pipeline.download_audio_file", return_value=b"audio"
    ), patch(
        "app.services.pipeline.transcribe_audio",
        side_effect=TranscriptionError("Groq 500 error"),
    ):
        with pytest.raises(PipelineError) as exc:
            process_meeting_pipeline(meeting_id=meeting_id, supabase_client=mock_db)
...
def test_process_meeting_pipeline_summarization_retry_success():
    ...
    valid_output = MeetingSummaryOutput(
        summary="Success on retry.", key_points=[], decisions=[], action_items=[]
    )

    with patch(
        "app.services.pipeline.download_audio_file", return_value=b"audio"
    ), patch(
        "app.services.pipeline.transcribe_audio", return_value=("Transcript", 1.0)
    ), patch(
        "app.services.pipeline.generate_meeting_summary",
        side_effect=[
            SummarizationError(
                "Pydantic structured output validation failed on Gemini response"
            ),
            (valid_output, 1.5),
        ],
    ) as mock_sum:
...
def test_process_meeting_pipeline_summarization_retry_failure_api():
    ...
    with patch(
        ...
        "app.services.pipeline.generate_meeting_summary",
        side_effect=[Exception("Gemini quota 503"), Exception("Gemini quota 503")],
    ):
...
def test_process_meeting_pipeline_db_failure_resilience():
    ...
    with patch(
        ...
        "app.services.pipeline.transcribe_audio",
        side_effect=TranscriptionError("ASR error"),
    ):
...
def test_process_meeting_pipeline_concurrency_isolation():
    ...
    for m_id in ["meet-A", "meet-B"]:
        ...
        with patch(
            ...
            "app.services.pipeline.transcribe_audio",
            return_value=(f"Transcript for {m_id}", 1.0),
        ), patch(
            "app.services.pipeline.generate_meeting_summary",
            return_value=(MeetingSummaryOutput(summary=f"Summary for {m_id}"), 1.0),
        ):
