...
def test_normalize_transcript_preserves_content_and_punctuation():
    ...
    expected = "Speaker 1: Are we on track for Q3? Yes, 100% on track!\nSpeaker 2: Great."
    assert normalize_transcript(raw) == expected
...
def test_transcribe_audio_api_failure():
    mock_groq = MagicMock()
    mock_groq.audio.transcriptions.create.side_effect = Exception("Groq API rate limit exceeded")
...
def test_process_transcription_for_meeting_success():
    ...
    mock_db.table.return_value.select.return_value.eq.return_value.execute.return_value = mock_select_response
...
def test_process_transcription_for_meeting_storage_download_failure():
    ...
    mock_db.table.return_value.select.return_value.eq.return_value.execute.return_value = mock_select_response
    mock_db.storage.from_.return_value.download.side_effect = Exception("Storage file missing")
