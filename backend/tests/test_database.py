...
def test_create_meeting_record_failure(mock_supabase):
    ...
    mock_supabase.table.return_value.insert.return_value.execute.side_effect = Exception("DB Connection Timeout")
...
def test_update_meeting_status_success(mock_supabase):
    ...
    mock_supabase.table.return_value.update.assert_called_with({"status": "TRANSCRIBING"})
