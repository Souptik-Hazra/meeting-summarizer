...
def _safe_update_status(
    ...
    try:
        ...
        logger.error(
            f"Critical: Failed to update meeting status to '{status.value}' for {meeting_id}: {str(db_err)}"
        )
...
def process_meeting_pipeline(
    ...
    try:
        ...
        raise PipelineError(
            "Database failure retrieving initial meeting record.",
    ...
    if not record:
        logger.error(
            f"Meeting with ID '{meeting_id}' not found for background pipeline."
        )
        raise PipelineError(
            f"Meeting with ID '{meeting_id}' not found.", failure_stage="storage"
        )

    current_status = record.get("status")
    # Prevent duplicate / invalid concurrent execution
    if (
        current_status != MeetingStatus.PENDING.value
        and current_status != MeetingStatus.PENDING
    ):
        logger.warning(
            f"Meeting {meeting_id} is in status '{current_status}'. Skipping pipeline execution."
        )
        return record
    ...
    if not storage_path:
        ...
        raise PipelineError(
            "Missing audio storage path in meeting record.", failure_stage="storage"
        )
    ...
    try:
        ...
        raise PipelineError(
            "Failed to retrieve audio file from storage.",
            failure_stage="storage",
            original_error=e,
        )
    except Exception as e:
        ...
        raise PipelineError(
            "Unexpected cloud storage error.", failure_stage="storage", original_error=e
        )

    try:
        whisper_client = groq_client or get_groq_client()
        raw_transcript, transcription_time = transcribe_audio(
            audio_bytes=audio_bytes, filename=original_filename, client=whisper_client
        )
        ...
        logger.error(
            f"Unexpected transcription error for meeting {meeting_id}: {str(e)}"
        )
        _safe_update_status(
            meeting_id=meeting_id,
            status=MeetingStatus.FAILED,
    ...
    _safe_update_status(
        meeting_id=meeting_id, status=MeetingStatus.SUMMARIZING, client=db
    )
    ...
    try:
        ...
        logger.warning(
            f"Summarization Attempt 1 failed for meeting {meeting_id}: {str(exc1)}. Retrying once in 1.5s..."
        )
        time.sleep(1.5)
        ...
        try:
            ...
            if "validation" in err_str2 or "structured output" in err_str2:
                is_validation_failure = True
            logger.error(
                f"Summarization Attempt 2 (Retry) failed for meeting {meeting_id}: {str(exc2)}"
            )
    ...
    if not summary_output:
        ...
        safe_msg = (
            ...
            else "Generative AI summarization failed."
        )
        _safe_update_status(
            meeting_id=meeting_id,
            status=MeetingStatus.FAILED,
        ...
        raise PipelineError(
            safe_msg, failure_stage=stage, original_error=last_summarization_error
        )
    ...
    try:
        ...
        logger.error(
            f"Failed to persist final completed intelligence for meeting {meeting_id}: {str(e)}"
        )
        _safe_update_status(
            meeting_id=meeting_id,
            status=MeetingStatus.FAILED,
        ...
        raise PipelineError(
            "Database failure persisting final meeting record.",
            failure_stage="summarization",
            original_error=e,
        )
