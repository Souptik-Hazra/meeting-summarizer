...
def get_supabase_client() -> Client:
    ...
    try:
        ...
        raise DatabaseError("Failed to establish connection to database service.", original_error=e)
...
def create_meeting_record(
    ...
    payload = {
        ...
        "status": meeting_data.status.value if isinstance(meeting_data.status, MeetingStatus) else meeting_data.status,
    }

    try:
        response = db.table("meetings").insert(payload).execute()
        if not response.data or len(response.data) == 0:
            raise DatabaseError(f"No record returned after creating meeting: {meeting_data.meeting_id}")
        return response.data[0]
    except DatabaseError:
        raise
    except Exception as e:
        logger.error(f"Database error creating meeting record {meeting_data.meeting_id}: {str(e)}")
        raise DatabaseError("Failed to create meeting persistence record.", original_error=e)
...
def get_meeting_record(
    ...
    db = client or get_supabase_client()
    try:
        response = db.table("meetings").select("*").eq("meeting_id", meeting_id).execute()
        if not response.data or len(response.data) == 0:
            return None
        return response.data[0]
    except Exception as e:
        logger.error(f"Database error retrieving meeting record {meeting_id}: {str(e)}")
        raise DatabaseError(f"Failed to fetch meeting record for {meeting_id}.", original_error=e)
...
def update_meeting_status(
    ...
    try:
        response = db.table("meetings").update(update_payload).eq("meeting_id", meeting_id).execute()
        if not response.data or len(response.data) == 0:
            return None
        return response.data[0]
    except Exception as e:
        logger.error(f"Database error updating meeting status {meeting_id}: {str(e)}")
        raise DatabaseError(f"Failed to update status for meeting {meeting_id}.", original_error=e)
...
def update_meeting_record(
    ...
    db = client or get_supabase_client()
    try:
        response = db.table("meetings").update(update_fields).eq("meeting_id", meeting_id).execute()
        if not response.data or len(response.data) == 0:
            return None
        return response.data[0]
    except Exception as e:
        logger.error(f"Database error updating meeting record {meeting_id}: {str(e)}")
        raise DatabaseError(f"Failed to update meeting record for {meeting_id}.", original_error=e)
